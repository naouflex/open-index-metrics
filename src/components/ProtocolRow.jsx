...
export default function ProtocolRow({ 
  ...
  // For AAVE protocol, exclude Balancer data due to incorrect subgraph data
  const balancerTVL = useBalancerTVL(
    protocol.ticker !== 'FRAX' && protocol.ticker !== 'AAVE' ? protocol.govContractAddress : null,
    { enabled: shouldLoad && protocol.ticker !== 'FRAX' && protocol.ticker !== 'AAVE' }
  );
  ...
  const dexLiquidityToMc = marketCap > 0 ? totalDexTVL / marketCap : 0;
    ...
    >
      ...
      {visibleColumns.dexLiquidityToMc && (
        ...
        >
          <Text
            fontSize="sm"
            color={getColorForMetric(dexLiquidityToMc * 100, 'dexLiquidityToMc', colorMode)}
            fontWeight="semibold"
          >
            {formatPercentage(dexLiquidityToMc)}
          </Text>
        </Td>
      )}

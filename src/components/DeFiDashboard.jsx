...
const COLUMN_DEFINITIONS = {
  ...
  dexLiquidityToMc: 'DEX Liquidity / % of Mkt Cap',
  tvlToFdv: 'TVL / FDV (%)',
...
export default function DeFiDashboard() {
  ...
  const allBalancerTVL = protocols.map(protocol => 
    useBalancerTVL(
      protocol.ticker !== 'FRAX' && protocol.ticker !== 'AAVE' ? protocol.govContractAddress : null, 
      { enabled: allProtocolsLoaded && protocol.ticker !== 'FRAX' && protocol.ticker !== 'AAVE' }
    )
  );
  ...
  const protocolsWithData = useMemo(() => {
    return protocols.map((protocol, index) => {
      ...
      const dexLiquidityToMc = marketCap > 0 ? mainnetDexTVL / marketCap : 0;
      ...
      if (protocolInfoData?.data?.tvl && Array.isArray(protocolInfoData.data.tvl)) {
        ...
        if (tvlHistory.length > 0 && currentTvl > 0) {
          ...
          if (closestTvlPoint && closestTvlPoint.totalLiquidityUSD > 0) {
            ...
            tvlGrowthMonthlyAvg = (Math.pow(currentTvl / closestTvlPoint.totalLiquidityUSD, 1 / 12) - 1) * 100;
          }
        }
      }
        ...
        >
          <Thead bg={tableHeaderBg} position="sticky" top={0} zIndex={3}>
            <Tr>
              ...
              {visibleColumns.dexLiquidityToMc && (
                <SortableHeader 
                  column="dexLiquidityToMc" 
                  currentSort={sortConfig} 
                  onSort={handleSort} 
                  onReset={handleReset} 
                  dataSource="calc" 
                  fontSize="xs"
                  minW={{ base: "90px", sm: "105px", md: "120px", lg: "135px" }}
                  maxW={{ base: "90px", sm: "105px", md: "120px", lg: "135px" }}
                  w={{ base: "90px", sm: "105px", md: "120px", lg: "135px" }}
                >
                  DEX Liquidity / % of Mkt Cap
                </SortableHeader>
              )}
